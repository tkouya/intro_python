# graph/app.py : 関数グラフの描画
from flask import Flask
from flask import render_template
from flask import request # methods as argument
from flask import redirect
from flask import Response
from numpy import * # NumPyの関数を使用する
import numexpr as ne # 式パーサ
...
def draw():
    ...
    # 計算とグラフ描画
    x_array = linspace(float(x_min), float(x_max), div)
    y_array = [ne.evaluate(formula_text) for x in x_array]
    fig, ax = plt.subplots()
    ax.plot(x_array, y_array)
    fig.savefig(graph_img_name)
    return render_template('index.html', formula = formula_text, x_min = x_min, x_max = x_max, img_name = graph_img_name)
